...
function calculateTotalPrice() {
  ...
  totalPrice = basketContent.reduce((sum, item) => sum + item.price * item.quantity, 0);
  totalPriceWithDelivery = deliverySelected ? totalPrice : totalPrice + deliveryCost;
}
...
function getFromLocalStorage() {
  ...
  if (basket) {
    basketContent = JSON.parse(basket).map(item => ({ ...item, quantity: item.quantity || 1 }));
  } else {
    basketContent = [];
  }
}
...
function displayBasket() {
  ...
  basketSection.innerHTML = basketContent.length === 0
...
function createOrderConfirmationHTML({ items, totalPrice, deliverySelected }) {
  ...
  const listItems = items.map(
    i => `<li>${i.name} - ${i.quantity} Stück - ${(i.price * i.quantity).toFixed(2)} €</li>`
  ).join("");
